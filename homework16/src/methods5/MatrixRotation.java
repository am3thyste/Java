...
public class MatrixRotation {
	 ...
	 public static void main(String[] args) {
			...
			if (rotateMatrix(matrix, mode)) {
				String modeSwitch = String.valueOf(mode);
				printMatrixToConsole(matrix);
				switch(modeSwitch) {
					case "1":
						rotate90(matrix);
					...
					case "3":
						...
						break;
					default:
						System.out.println("no such option for switch");
				}
				}			
		}
	 ...
	 public static boolean rotateMatrix(double[][]matrix, int mode) {
		...
		public static double[][] generateMatrix(int size) {
			BigDecimal a = BigDecimal.valueOf(0).setScale(1); 
			BigDecimal b = BigDecimal.valueOf(0.1).setScale(1);
			BigDecimal c = BigDecimal.valueOf(1).setScale(0);
			BigDecimal counter = BigDecimal.valueOf(0).setScale(0);
			double[][] matrix = new double[size][size];
			for (int i = 0; i < size; i++) {
				for (int j=0; j<size; j++) {
					matrix[i][j] = a.doubleValue(); 
					a = a.add(b);
					counter = counter.add(c);
			}
				a = a.add(c).subtract(b.multiply(counter));
				counter = BigDecimal.valueOf(0);
			}
			return matrix;
			}
