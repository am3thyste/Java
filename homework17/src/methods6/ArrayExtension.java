package methods6;
	...
	public class ArrayExtension {
		...
		public static int[] extendArray(int[] arr) {
			...
			for (int i = 0; i < extendedArray.length; i++) {
				if (i< arrLength) {
					...
					extendedArray[i] = arr[i-arrLength]*2;
				}
			}
		...
		public static int[] generateRandomArray(int amountOfElements) {
			int[] generatedRandonArray = new int[amountOfElements];
			for (int i = 0; i < amountOfElements; i++) {
				generatedRandonArray[i] = (int)(Math.random() * (100-1));
			}
			return generatedRandonArray;
		}
